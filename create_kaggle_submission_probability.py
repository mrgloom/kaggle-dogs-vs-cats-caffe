...
def write_array_kaggle_format(predictions):
    ...
    text_file = open(submission_dir+'kaggle_submit_probability_'+MODEL_POSTFIX+".csv", "w")
    
    #write header
    text_file.write('id,label\n')
        
    for i in range(0,predictions.shape[0]):
        text_file.write("%i,%.4f\n" % (i+1,predictions[i])) #probability that the image is a dog [0,1]
...
def create_kaggle_submission(test_data_path):
    ...
    predictions= np.zeros((len(filenames), 1), dtype=np.float32)
    ...
    for i in range(0,len(filenames),BATCH_SIZE):
        if(i+BATCH_SIZE < len(filenames)):
            ...
            predictions[i:i+BATCH_SIZE]= predict_proba(images_batch)
        else:
            bz= len(filenames)-i
            #print i, i+bz
            images_batch= load_batch(filenames[i:i+bz])
            predictions[i:i+bz]= predict_proba(images_batch)
